 ...
 *
 * Runs on Cloudflare Workers and is served at https://mcp.icemail.ai
 *   - Streamable HTTP transport:  https://mcp.icemail.ai/mcp   (current standard)
 *   - SSE transport (legacy):     https://mcp.icemail.ai/sse   (older clients)
 *
 * AUTH: each user connects with THEIR OWN Icemail API key, passed on every
 * request as `Authorization: Bearer <ICEMAIL_API_KEY>`. The Worker forwards
 * that key to the Icemail REST API — it stores nothing and has no shared
 * server-side secret.
 *
 ...
 * tool — the transport, auth, and hosting layers do not change.
 */
...
/** Per-connection auth, injected from the Authorization header (see fetch handler). */
type Props = {
  apiKey: string;
};
...
export class IcemailMCP extends McpAgent<Env, unknown, Props> {
  ...
  /** Call the Icemail REST API with the caller's key. */
  private async icemailFetch(
    ...
    if (!apiKey) {
      throw new MissingKeyError(
        "No Icemail API key provided. Connect with an `Authorization: Bearer <key>` header. Create a key at https://app.icemail.ai/settings/api.",
      );
    }

    const base = this.env.ICEMAIL_API_BASE.replace(/\/$/, "");
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    // Inject the caller's Icemail key so the agent can read it via this.props.
    // McpAgent reads `ctx.props` at runtime; the type marks it readonly, so cast.
    (ctx as { props?: Props }).props = { apiKey: extractApiKey(request) };
    ...
    // Health check.
    if (pathname === "/health") {
      return Response.json({ ok: true, service: "icemail-mcp" });
    }
    ...
    if (pathname === "/" || pathname === "") {
      return new Response(
        [
          ...
          "  https://mcp.icemail.ai/mcp   (Streamable HTTP — recommended)",
          "  https://mcp.icemail.ai/sse   (SSE — legacy clients)",
          "",
          "Authenticate with your Icemail API key:",
          "  Authorization: Bearer <ICEMAIL_API_KEY>",
          "",
          "Docs: https://docs.icemail.ai/mcp-server",
        ].join("\n"),
