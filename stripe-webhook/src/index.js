...
import { cioIdentify, cioTrackPayment } from "./customerio.js";
  ...
  async fetch(request, env, ctx) {
    ...
    if (event.type === "customer.created") {
      ...
      if (toEmail) {
        ...
        // Customer.io sync: create the profile (signup date + name). With no
        // payment event yet, the rule-based segments place them in "Free Trial".
        if (env.CIO_SITE_ID && env.CIO_TRACK_API_KEY) {
          ctx.waitUntil(
            cioIdentify(env, {
              email: toEmail,
              name: customer.name,
              ...
              console.error(`CIO identify failed for ${toEmail}:`, e.message)
            )
          );
        }
      }
    ...
    // Customer.io sync: a successful charge records a `payment` event, moving
    // the person Free Trial -> Paid. Customer.io's 45-day rolling window then
    // ages them Paid -> Churned automatically with no further action.
    if (event.type === "charge.succeeded") {
      if (env.CIO_SITE_ID && env.CIO_TRACK_API_KEY) {
        ctx.waitUntil(handlePaymentForCio(env, stripe, event.data.object));
      }
    }
...
// Customer.io: on charge.succeeded, resolve the customer's canonical email
// (so the payment lands on the same profile the identify created), upsert the
// profile, then record the `payment` event at the charge's real timestamp.
async function handlePaymentForCio(env, stripe, charge) {
  try {
    ...
    if (charge.customer) {
      try {
        ...
        if (customer && !customer.deleted) {
          ...
          createdAt = customer.created || null;
        }
      } catch (e) {
        console.error("CIO: customer retrieve failed:", e.message);
      }
    }

    if (!email) {
      console.error(`CIO: charge ${charge.id} has no email, skipping.`);
      return;
    }

    // Upsert keeps identity consistent even if customer.created was missed.
    await cioIdentify(env, { email, name, createdAt });
    await cioTrackPayment(env, { email, timestamp: charge.created });
  } catch (e) {
    console.error("CIO payment sync failed:", e.message);
  }
}
